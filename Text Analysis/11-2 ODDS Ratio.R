## 텍스트 분석 11-2 tidy ODDS Ratio
# 상대적인 단어 중요도를 나타내는 방법론

library(dplyr)
# 문재인 대통령 연설문 불러오기

file <- file.choose() #speech_moon.txt 불러옴
...
# 박근혜 대통령 연설문 불러오기
file <- file.choose() #speech_part.txt 불러옴
raw_park <- readLines(file, encoding = "UTF-8")
park <- raw_park %>%
  as_tibble() %>%
